...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .tag-pill { display: inline-flex; align-items: center; padding: 2px 10px; border-radius: 999px; font-size: 11px; font-weight: 600; }
  ...
  .timetable-cell { border: 1px solid ${G.border}; padding: 4px; min-height: 56px; border-radius: 6px; background: ${G.surface}; overflow: hidden; }
...
function Announcements({ user }) {
  ...
  const submit = async () => {
    ...
    setLoading(true);
    await addDoc(collection(db, "announcements"), { title, body, tag, pinned, author: user.displayName || user.email, uid: user.uid, createdAt: serverTimestamp() });
    setTitle(""); setBody(""); setTag("#General"); setPinned(false); setShowForm(false); setLoading(false);
  };

  const del = async (id, uid) => { if (uid !== user.uid) return; await deleteDoc(doc(db, "announcements", id)); };
        ...
        {ordered.map((p, i) => (
          <div key={p.id} className="card slide-up" style={{ borderLeft: `3px solid ${TAG_COLORS[p.tag] || G.accent}`, animationDelay: `${i * 0.05}s` }}>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
              ...
              {p.uid === user.uid && <button className="btn-danger" onClick={() => del(p.id, p.uid)}>Delete</button>}
            </div>
          </div>
        ))}
      </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {TAGS.map(t => (
                    <button key={t} onClick={() => setTag(t)} style={{ padding: "4px 12px", borderRadius: 999, border: `1px solid ${tag === t ? TAG_COLORS[t] : G.border}`, background: tag === t ? TAG_COLORS[t] + "25" : "transparent", color: tag === t ? TAG_COLORS[t] : G.muted, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>{t}</button>
                  ))}
                </div>
              </div>
...
function Timetable({ user }) {
  ...
  useEffect(() => {
    const u1 = onSnapshot(collection(db, "units"), snap => setUnits(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
    const u2 = onSnapshot(collection(db, "slots"), snap => setSlots(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
    return () => { u1(); u2(); };
  }, []);
  ...
  const addUnit = async () => {
    ...
    setUnitName(""); setUnitCode(""); setUnitLecturer(""); setShowUnitForm(false); setLoading(false);
  };

  const delUnit = async (id) => {
    await deleteDoc(doc(db, "units", id));
    await Promise.all(slots.filter(s => s.unitId === id).map(s => deleteDoc(doc(db, "slots", s.id))));
  };
  ...
  const getSlotForCell = (day, hour) => slots.filter(s => s.day === day && s.start <= hour && s.end > hour);
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div className="card" style={{ borderColor: currentSlot ? G.accent : G.border }}>
              ...
              {currentSlot ? (<><p style={{ fontWeight: 600, fontSize: 14 }}>{getUnit(currentSlot.unitId)?.code}</p><p style={{ color: G.muted, fontSize: 12 }}>{currentSlot.venue || "No venue"}</p><p style={{ color: G.muted, fontSize: 11, marginTop: 4 }}>Until {timeStr(currentSlot.end)}</p></>) : <p style={{ color: G.muted, fontSize: 13 }}>Free period</p>}
            </div>
            <div className="card">
              <p style={{ fontSize: 10, color: G.muted, fontWeight: 700, marginBottom: 6 }}>NEXT</p>
              {nextSlot ? (<><p style={{ fontWeight: 600, fontSize: 14 }}>{getUnit(nextSlot.unitId)?.code}</p><p style={{ color: G.muted, fontSize: 12 }}>{nextSlot.venue || "No venue"}</p><p style={{ color: G.muted, fontSize: 11, marginTop: 4 }}>{timeStr(nextSlot.start)}</p></>) : <p style={{ color: G.muted, fontSize: 13 }}>Nothing more today</p>}
            </div>
          </div>
        ...
        <div style={{ overflowX: "auto" }}>
          <div style={{ minWidth: 700 }}>
            <div style={{ display: "grid", gridTemplateColumns: "60px repeat(5, 1fr)", gap: 4, marginBottom: 4 }}>
              <div />
              {DAYS.map(d => <div key={d} style={{ textAlign: "center", fontSize: 12, fontWeight: 700, color: d === todayName ? G.accent : G.muted, padding: "6px 0" }}>{d.slice(0, 3)}</div>)}
            </div>
            {HOURS.map(h => (
      ...
      {showSlotForm && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowSlotForm(false)}>
          <div className="modal-box slide-up">
            ...
            {units.length === 0 ? <p style={{ color: G.warn }}>Please add at least one unit first.</p> : (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <nav style={{ background: G.surface, borderBottom: `1px solid ${G.border}`, padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 60, position: "sticky", top: 0, zIndex: 50 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
            ...
            <div style={{ display: "flex", gap: 4 }}>
              {[{ id: "announcements", label: "Announcements", icon: "📢" }, { id: "timetable", label: "Timetable", icon: "📅" }].map(t => (
